...
def get_trainer_list(db: Session, session: dict, skip: int = 0, limit: int = 10):
    _trainer_list = db.query(Trainer).join(Trainer.picture,isouter=True).filter(Trainer.branch_id == session['branch_id'],Trainer.is_trainer == True,Trainer.enable == True).order_by(Trainer.id.desc())
