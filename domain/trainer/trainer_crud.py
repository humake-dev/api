...
def get_trainer_list(db: Session, session: dict, skip: int = 0, limit: int = 10):
    _trainer_list = db.query(Trainer).join(Trainer.trainer_picture,isouter=True).filter(Trainer.branch_id == session['branch_id']).order_by(Trainer.id.desc())
...
def get_trainer(db: Session, session: dict, trainer_id: int):
