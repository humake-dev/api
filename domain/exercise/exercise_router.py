...
def exercise_list(db: Session = Depends(get_db), session: dict = Depends(get_session), page: int = 0, size: int = 10):
    total, _exercise_list = exercise_crud.get_exercise_list(db, session, skip=page*size, limit=size)
    return {
        'total': total,
...
def exercise_detail(exercise_id: int, session: dict = Depends(get_session), db: Session = Depends(get_db)):
    exercise = exercise_crud.get_exercise(db, session, exercise_id=exercise_id)
