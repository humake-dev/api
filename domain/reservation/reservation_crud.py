...
from models import ReservationUser,Reservation,Trainer
from sqlalchemy import select, func, extract
from sqlalchemy.orm import Session

def get_reservation_list(db: Session, session: dict, filters: dict = {}, skip: int = 0, limit: int = 10):
    conditions = [
        ReservationUser.user_id == session['user_id'],
...
def get_reservation(db: Session, session: dict, id: int):
    reservation = db.query(ReservationUser).join(Reservation).filter(ReservationUser.user_id == session['user_id']).filter(Reservation.id == id).first()
    return reservation