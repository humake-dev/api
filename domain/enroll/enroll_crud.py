...
def get_enroll_list(db: Session, session: dict, skip: int = 0, limit: int = 10):
    _enroll_list = db.query(Enroll).join(Order).filter(Order.user_id == session['user_id'],Order.enable == True,Enroll.start_date < date.today(),Enroll.end_date > date.today()).order_by(Enroll.id.desc())
...
def get_enroll(db: Session, session: dict, id: int):
    trainer = db.query(Enroll).get(id)
    return trainer